...
export async function boostPost(postId: string, userId: string, budget: number, multiplier: number) {
  if (![2, 3, 5].includes(multiplier)) throw new Error("المضاعف المسموح: 2x أو 3x أو 5x");
  const minBudget = 50;
  if (budget < minBudget) throw new Error("الحد الأدنى 50 نقطة");
  ...
  await runTransaction(db, async (transaction) => {
    ...
    // Update post with boosted flag first to calculate correct score
