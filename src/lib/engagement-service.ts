...
import { trackPlatformActivity, ANTI_ABUSE_LIMITS, ANTI_FRAUD_LIMITS } from "./platform-service";
import { extractHashtags } from "./hashtag-service";
...
/**
 * صرف مكافآت التفاعل مع تطبيق تقسيم 51/49
 */
export async function rewardEngagement(
  ...
  if (!postData || !userData) return { success: false, message: "بيانات غير موجودة" };
  ...
  const finalAmount = isSuspicious ? 0 : (amount * multiplier);
  ...
  return await runTransaction(db, async (t) => {
    ...
    const userUpdates: any = {
      dailyEngagement: {
        date: today,
        secondsUsed: (daily.date === today ? daily.secondsUsed : 0) + seconds,
        pointsEarnedToday: increment(finalAmount)
      }
    };

    if (finalAmount > 0) {
      // تطبيق تقسيم 51/49
      const withdrawablePart = Math.floor(finalAmount * 0.51);
      const lockedPart = finalAmount - withdrawablePart;

      userUpdates.totalPoints = increment(finalAmount);
      userUpdates.withdrawablePoints = increment(withdrawablePart);
      userUpdates.lockedPoints = increment(lockedPart);
    }
    ...
    if (multiplier > 1 && !isSuspicious) {
      postUpdates['boostConfig.remainingBudget'] = increment(-finalAmount);
    ...
    updatePostInCache(postId, postUpdates);

    return { success: true, reward: finalAmount };
  });
}