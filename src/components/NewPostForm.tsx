...
export default function NewPostForm({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="p-6 bg-card max-h-[90vh] overflow-y-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <Textarea
          ...
          className="min-h-[150px] bg-background border-border rounded-2xl"
          value={text}
        ...
        {showLinkInput && <Input placeholder="رابط خارجي..." value={link} onChange={(e) => setLink(e.target.value)} className="rounded-xl" />}
        ...
        <Button type="submit" disabled={loading || !text.trim()} className="w-full h-12 rounded-xl text-lg font-bold gap-2">
