      ...
      <DialogContent className="rounded-3xl bg-card border-border max-w-[95vw] sm:max-w-[450px]">
        <DialogHeader>
          ...
          <DialogDescription className="text-right text-xs leading-relaxed">
            استخدم رصيدك الإعلاني لزيادة ظهور منشورك ومضاعفة مكافآت المتفاعلين لجذب انتباه أكبر.
          </DialogDescription>
        </DialogHeader>
              ...
              <div className="grid grid-cols-3 gap-2">
                 {[2, 3, 5].map(m => (
                    <Button 
                      ...
                      className={`h-12 rounded-xl font-black text-lg ${multiplier === m ? 'bg-primary border-primary' : 'border-border'}`}
                    >
                       {m}x
                    </Button>
                 ))}
