    ...
    <div className="max-w-2xl mx-auto px-4 pt-6 pb-24">
      ...
      <div className="pulse-card rounded-3xl p-6 text-white mb-8 relative overflow-hidden">
        <div className="relative z-10">
          ...
          <h2 className="text-3xl font-black mt-2">نخبة المجتمع</h2>
        </div>
        ...
        {leaders.map((leader, index) => (
          <div key={leader.id} className={cn(
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="font-black text-lg">{leader.displayName}</p>
                <p className="text-xs text-muted-foreground">عضو منذ {leader.createdAt?.seconds ? new Date(leader.createdAt.seconds * 1000).toLocaleDateString('ar-SA') : "قريباً"}</p>
              </div>
            </div>
            <div className="text-left bg-background/50 px-4 py-2 rounded-xl border border-border">
